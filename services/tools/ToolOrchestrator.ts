...
export class ToolOrchestrator extends EventEmitter {
  ...
  private activeExecutions: Map<string, OrchestrationExecutionStatus> =
    new Map();
  ...
  private calculatePriority(plan: ToolOrchestrationPlan): number {
    ...
    // 根据工具状态调整
    const readyTools = plan.requiredTools.filter(
      tool => this.toolRegistry.getToolStatus(tool) === ToolStatus.READY
    );
  ...
  private async executePlanSteps(
    ...
    for (const step of steps) {
      ...
      try {
        ...
        // 更新进度
        execution.progress =
          (execution.completedSteps.length / steps.length) * 100;
  ...
  private areDependenciesSatisfied(
    ...
    return step.dependencies.every(
      dep =>
        execution.completedSteps.includes(dep) || execution.results.has(dep)
    );
  }
  ...
  getPerformanceStats() {
    ...
    const successfulExecutions = completedExecutions.filter(
      exec => exec.failedSteps.length === 0
    );
