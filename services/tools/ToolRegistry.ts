...
export class ToolRegistry extends EventEmitter {
    ...
    try {
      ...
      if (toolDefinition.capabilities) {
        const capabilitySet = new Set(
          toolDefinition.capabilities.map(cap => cap.name)
        );
      ...
      if (toolDefinition.dependencies) {
        this.dependencies.set(
          toolDefinition.name,
          new Set(toolDefinition.dependencies)
        );
      }
  ...
  async searchTools(query: {
    ...
    for (const tool of this.tools.values()) {
      ...
      if (query.capabilities && query.capabilities.length > 0) {
        ...
        matches =
          matches && query.capabilities.some(cap => toolCapabilities.has(cap));
      }
      ...
      if (query.tags && query.tags.length > 0) {
        matches = matches && query.tags.some(tag => tool.tags?.includes(tag));
      }
    ...
    return results.sort((a, b) => {
      ...
      const scoreA =
        (metricA?.qualityScore || 0) * (metricA?.executionCount || 0);
      const scoreB =
        (metricB?.qualityScore || 0) * (metricB?.executionCount || 0);
  ...
  async orchestrateTools(
    ...
    try {
      ...
      // 过滤可用工具
      const availableTools = relevantTools.filter(
        tool =>
          tool.status === ToolStatus.READY &&
          (!request.preferredTools ||
            request.preferredTools.includes(tool.name))
      );

      // 生成执行计划
      plan.steps = await this.generateExecutionSteps(
        request.goal,
        availableTools
      );
  ...
  async cleanupExpiredMetrics(): Promise<void> {
    ...
    cutoffDate.setDate(
      cutoffDate.getDate() - (this.config.metricsRetentionDays ?? 30)
    );
