...
export class KnowledgeBase extends EventEmitter {
  ...
  constructor(config: RAGConfig = {}) {
    ...
    // 初始化向量存储
    this.vectorStorage = new InMemoryVectorStorage(
  ...
  async addKnowledgeBatch(
    items: Array<Omit<KnowledgeItem, 'id' | 'createdAt' | 'updatedAt'>>
  ): Promise<string[]> {
    ...
    for (let i = 0; i < items.length; i += this.config.batchSize) {
      ...
      try {
        ...
        console.error(
          `批量添加知识失败 (批次 ${i / this.config.batchSize + 1}):`,
          error
        );
        throw error;
      }
      ...
      const results = similarItems
        ...
        .filter(item => {
          ...
          if (query.dateRange) {
            ...
            if (
              itemDate < query.dateRange.start ||
              itemDate > query.dateRange.end
            ) {
              return false;
            }
          }
  ...
  async updateKnowledge(
    ...
    try {
      ...
      if (updates.content || updates.title || updates.description) {
        const updatedText = this.concatenateText({
          ...existingItem,
          ...updates,
        });
  ...
  async importKnowledge(
    ...
    let items: Array<
      Omit<KnowledgeItem, 'id' | 'createdAt' | 'updatedAt' | 'embedding'>
    >;
  ...
  private generateSearchContext(
    ...
    const contextParts = [
      `基于查询 "${query.text}" 找到 ${results.length} 个相关知识条目:`,
    ];

    for (let i = 0; i < Math.min(3, results.length); i++) {
      const item = results[i];
      contextParts.push(
        `${i + 1}. ${item.title}: ${item.description || item.content.substring(0, 100)}...`
      );
    }
