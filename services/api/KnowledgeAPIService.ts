...
export class KnowledgeAPIService {
  ...
  private async handleGetPopularTags(request: Request): Promise<Response> {
    try {
      ...
      const tags = this.knowledgeManager
        .getTags()
  ...
  private async handleImportKnowledge(request: Request): Promise<Response> {
    try {
      ...
      const result = await this.knowledgeManager.importKnowledge(
        text,
        importOptions
      );
  ...
  private async handleGenerateKnowledgeGraph(
    request: Request
  ): Promise<Response> {
    try {
      const url = new URL(request.url);
      ...
      const minRelevanceScore = parseFloat(
        url.searchParams.get('minRelevance') || '0.5'
      );

      const graph = await this.knowledgeManager.generateKnowledgeGraph(
        maxNodes,
        minRelevanceScore
      );
