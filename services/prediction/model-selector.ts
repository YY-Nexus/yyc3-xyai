...
export class DynamicModelSelector {
  ...
  async evaluateModelFit(
    ...
    const biasVarianceTradeoff = this.analyzeBiasVariance(
      dataComplexity,
      modelComplexity
    );
  ...
  private analyzeDataCharacteristics(
    ...
    return {
      ...
      seasonality:
        data.dataType === 'timeseries' ? this.detectSeasonality(values) : null,
  ...
  private getCandidateModels(
    ...
    // 根据数据特征筛选模型
    return this.filterModelsByConstraints(
      candidates,
      dataCharacteristics,
      constraints
    );
  ...
  private filterModelsByConstraints(
    ...
    return models.filter(modelId => {
      ...
      // 训练时间约束
      if (
        constraints.maxTrainingTime &&
        modelInfo.averageTrainingTime > constraints.maxTrainingTime
      ) {
        return false;
      }
      ...
      if (
        ...
        modelInfo.memoryRequirement > constraints.memoryLimit
      ) {
        return false;
      }
      ...
      // 准确性要求
      if (
        constraints.accuracyThreshold &&
        modelInfo.expectedAccuracy < constraints.accuracyThreshold
      ) {
        return false;
      }
  ...
  private selectBestModel(
    ...
    // 选择得分最高的模型
    return scoredAssessments.reduce((best, current) =>
  ...
  private generateSelectionReason(
    ...
    if (
      model.biasVarianceTradeoff.bias < 0.3 &&
      model.biasVarianceTradeoff.variance < 0.3
    ) {
      reasons.push('偏差方差平衡');
    }

    if (
      dataCharacteristics.dataType === 'timeseries' &&
      model.residualAnalysis.autocorrelation < 0.2
    ) {
  ...
  private calculateMissingRate(data: PredictionData): number {
    ...
    for (const point of data.data) {
      ...
      if (point.features) {
        ...
        missingValues += Object.values(point.features).filter(
          v => v === null || isNaN(v)
        ).length;
      }
    }
  ...
  private detectSeasonality(values: number[]): SeasonalityInfo | null {
    ...
    for (const period of periods) {
      if (values.length >= period * 2) {
        const correlation = this.calculateSeasonalityCorrelation(
          values,
          period
        );
  ...
  private assessLinearity(data: PredictionData): number {
    ...
    const firstFeature = data.data.map(
      p => p.features?.[data.features[0]] || 0
    );
  ...
  private detectOutliers(values: number[]): number {
    ...
    const outlierCount = values.filter(
      v => Math.abs(v - stats.mean) > threshold
    ).length;
  ...
  private calculateDataComplexity(data: PredictionData): DataComplexity {
    return {
      ...
      seasonality:
        data.dataType === 'timeseries'
          ? this.detectSeasonality(data.data.map(p => p.value))
          : null,
  ...
  private calculateGoodnessOfFit(
    ...
    const complexityMatch =
      1 -
      Math.abs(dataComplexity.size - modelComplexity) /
        Math.max(dataComplexity.size, modelComplexity);
