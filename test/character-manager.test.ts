...
describe('Character Manager Tests', () => {
  ...
  test('should get AI avatar path correctly', () => {
    ...
    expect(femaleAvatar).toBe(
      '/role-photos/girl/ai-avatars/girl-xiaoyu-lolita-blue-001.png'
    );
    expect(maleAvatar).toBe(
  ...
  test('should get floating avatar path correctly', () => {
    ...
    const femaleFloatingAvatar =
      characterManager.getAIFloatingAvatarPath(femaleChar);
    const maleFloatingAvatar =
      characterManager.getAIFloatingAvatarPath(maleChar);

    expect(femaleFloatingAvatar).toBe(
      '/role-photos/girl/ai-avatars/girl-xiaoyu-lolita-pink-001.png'
    );
    expect(maleFloatingAvatar).toBe(
      '/role-photos/boy/ai-avatars/boy-xiaoyan-cool-001.png'
    );
  });
  ...
  test('should get character image path correctly', () => {
    ...
    // 特定主题图片
    const themeImage = characterManager.getCharacterImagePath(
      femaleChar,
      undefined,
      'pink'
    );
    ...
    // 特定表情图片
    const expressionImage = characterManager.getCharacterImagePath(
      femaleChar,
      'happy'
    );
    expect(expressionImage).toBe(
  ...
  test('should get theme colors correctly', () => {
    ...
    expect(themeColors.gradient).toBe(
  ...
  test('should get expressions and catchphrases correctly', () => {
    ...
    const happyExpression = characterManager.getCharacterExpression(
      femaleChar,
      'happy'
    );
