...
export default function DevelopmentCurveChart({
  ...
  // 数据验证和清理
  const validChildData = (actualChildData || []).filter(
    point =>
      point &&
      typeof point.age === 'number' &&
      !isNaN(point.age) &&
      point.age >= 0 &&
  ...
  const maxAge =
    validChildData.length > 0
      ? Math.max(...validChildData.map(d => d.age), 36)
      : 36;
  ...
  const yScale = (value: number) => {
    ...
    if (chartHeight <= 0 || maxValue <= minValue)
      return height - padding.bottom;
    const scaledValue =
      chartHeight -
  ...
  const latestPoint =
    validChildData && validChildData.length > 0
      ? validChildData[validChildData.length - 1]
      : null;
  const percentileRank = latestPoint
    ? Math.round(getPercentileRank(latestPoint.value, latestPoint.age))
    : null;
