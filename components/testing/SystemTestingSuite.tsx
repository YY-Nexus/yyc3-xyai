...
export default function SystemTestingSuite() {
  ...
  // 获取通过率
  const passRate =
    testStats.total > 0 ? (testStats.passed / testStats.total) * 100 : 0;
  ...
  const stopTests = () => {
    ...
    setTestResults(prev =>
      prev.map(test =>
        test.status === 'running'
          ? { ...test, status: 'pending', duration: 0 }
          : test
      )
    );
  };
            ...
            >
              ...
              <div className='p-6 border-b bg-gradient-to-r from-white to-gray-50'>
                <div className='flex items-center gap-3'>
                  <div
                    className={`w-12 h-12 bg-gradient-to-r ${suite.color} rounded-lg flex items-center justify-center`}
                  >
                              ...
                              <div className='text-right'>
                                ...
                                <div
                                  className={`text-sm font-medium ${
                                    parseFloat(String(test.metrics['actual'])) <
                                    parseFloat(String(test.metrics['target']))
                                      ? 'text-green-600'
                                      : 'text-red-600'
                                  }`}
                                >
                                  {String(test.metrics['actual'])} /{' '}
                                  {String(test.metrics['target'])}
                                </div>
                              </div>
                            )}
                          <div
                            className={`px-3 py-1 rounded-full text-sm ${getStatusColor(test.status)}`}
                          >
                            {test.status === 'passed'
                              ? '通过'
                              : test.status === 'failed'
