  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (mode === 'login') {
        ...
        setLocalError('密码重置功能暂未开放，请联系管理员');
      }
    } catch {
      setLocalError('操作失败，请稍后重试');
    }
  };
              ...
              {(error || localError) && (
                <div
                  className={`p-3 rounded-lg text-sm ${
                    localError?.includes('已发送')
                      ? 'bg-green-50 text-green-600'
                      : 'bg-red-50 text-red-600'
                  }`}
                >
                  <i
                    className={`${localError?.includes('已发送') ? 'ri-check-line' : 'ri-error-warning-line'} mr-2`}
                  />
                  {error || localError}
                </div>
              )}
