...
export default function ProjectExecutionManager() {
      ...
      tasks: [
        {
          ...
          title: '后端架构设计与搭建',
          description:
            '选择Node.js + Express + TypeScript技术栈，设计RESTful API架构，搭建数据库',
          status: 'pending',
          priority: 'critical',
      ...
      tasks: [
        {
          ...
          title: '本地化AI引擎建设',
          description:
            '集成Llama 2-7B模型，优化语音识别与合成，实现智能对话引擎',
          status: 'pending',
          priority: 'high',
          assignee: 'AI Team',
  ...
  const [activeView, setActiveView] = useState<'kanban' | 'list' | 'metrics'>(
    'kanban'
  );
            ...
            <div className='flex bg-white rounded-lg shadow-sm'>
              {['kanban', 'list', 'metrics'].map(view => (
                <button
                  key={view}
                  onClick={() =>
                    setActiveView(view as 'kanban' | 'list' | 'metrics')
                  }
