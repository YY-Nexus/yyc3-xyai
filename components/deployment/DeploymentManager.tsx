...
export default function DeploymentManager() {
  ...
  const [environments, setEnvironments] =
    useState<Environment[]>(defaultEnvironments);
  ...
  const executeDeployment = async () => {
    ...
    try {
      ...
      const failedStep = deploymentSteps.find(step =>
        deploymentStatus.find(
          status => status.id === step.id && status.status === 'running'
        )
      );
      if (failedStep) {
            ...
            {environments.map(env => (
              ...
              >
                <div className='flex items-center justify-between mb-2'>
                  ...
                  <div
                    className={`px-2 py-1 rounded-full text-xs ${getEnvironmentStatusColor(env.status)}`}
                  >
                    {env.status === 'active'
                      ? '活跃'
                      : env.status === 'inactive'
                ...
                <div className='text-sm text-gray-600 space-y-1'>
                  ...
                  {env.lastDeploy && (
                    <div className='flex items-center gap-1'>
                      ...
                      <span>
                        上次部署: {env.lastDeploy.toLocaleDateString()}
                      </span>
                    </div>
                  )}
                </div>
              ...
              >
                <div className='flex items-center justify-between'>
                  ...
                  <div className='flex items-center gap-4'>
                    ...
                    <div
                      className={`px-3 py-1 rounded-full text-sm ${getStatusColor(step.status)}`}
                    >
                      {step.status === 'completed'
                        ? '已完成'
                        : step.status === 'failed'
                            ...
                            className={`flex items-center gap-2 text-xs p-2 rounded ${
                              step.status === 'completed'
                                ...
                                : step.status === 'running' &&
                                    checkIndex <
                                      Math.floor(
                                        (step.progress / 100) * checks.length
                                      )
            ...
            <div className='mt-8 grid grid-cols-1 md:grid-cols-4 gap-4'>
              <div className='bg-white rounded-lg p-4 text-center'>
                <div className='text-2xl font-bold text-green-600'>
                  {
                    deploymentStatus.filter(s => s.status === 'completed')
                      .length
                  }
                </div>
              ...
              <div className='bg-white rounded-lg p-4 text-center'>
                <div className='text-2xl font-bold text-yellow-600'>
                  {deploymentStatus.filter(s => s.status === 'warning').length}
                </div>
                <div className='text-sm text-gray-600'>警告步骤</div>
              </div>
              <div className='bg-white rounded-lg p-4 text-center'>
                <div className='text-2xl font-bold text-blue-600'>
                  {deploymentStatus.filter(s => s.status === 'failed').length}
                </div>
              ...
              <div className='bg-white rounded-lg p-4 text-center'>
                <div className='text-2xl font-bold text-purple-600'>
                  {deployStartTime
                    ? ((Date.now() - deployStartTime.getTime()) / 1000).toFixed(
                        1
                      ) + 's'
                    : '0s'}
                </div>
