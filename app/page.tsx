  ...
  useEffect(() => {
    const loadStats = async () => {
      ...
      const [records, milestones, assessments] = await Promise.all([
        db.count(
          'growth_records',
          (r: { child_id: string }) => r.child_id === currentChild.id
        ),
        db.count(
          'milestones',
          (m: { child_id: string }) => m.child_id === currentChild.id
        ),
        db.count(
          'growth_assessments',
          (a: { child_id: string }) => a.child_id === currentChild.id
        ),
      ]);
        ...
        <section className='max-w-7xl mx-auto w-full px-8 pb-28 pt-2 grid grid-cols-12 gap-8'>
          ...
          >
                ...
                >
                  ...
                  <div className='flex-1'>
                    ...
                    <p className='text-xs text-slate-400'>
                      {currentChild.current_stage || '成长中'}
                    </p>
                  </div>
            ...
            <div className='mt-4 text-center'>
              <h2 className='text-2xl font-rounded font-bold text-slate-700'>
                快乐学习助手
              </h2>
          ...
          >
            ...
            {currentChild ? (
              <HomeworkCenterCard
                childName={currentChild.nickname || currentChild.name}
              />
            ) : (
              <HomeworkCenterCard />
            )}
      ...
      <div className='grid grid-cols-1 md:grid-cols-2 gap-4 z-10'>
        {plans.map(plan => (
          ...
          >
            ...
            <span
              className={`font-medium ${plan.done ? 'text-slate-400 line-through' : 'text-slate-700'}`}
            >
              {plan.text}
            </span>
          </motion.div>
  ...
  return (
    ...
    >
      <div className='flex flex-col gap-2'>
        <h2 className='text-3xl font-rounded font-extrabold text-slate-800 flex items-center gap-2'>
          作业中心
        </h2>
  ...
  return (
    <Link href={href}>
      ...
      >
        ...
        >
          <i
            className={`${icon} ${colorClasses[color].split(' ')[1]} text-2xl`}
          />
        </motion.div>
