...
export default function BadgesPage() {
  ...
  const categories = [
    'all',
    ...Array.from(new Set(mockBadges.map(b => b.category))),
  ];

  const filteredBadges = mockBadges.filter(badge => {
    const filterMatch =
      filter === 'all' ||
      (filter === 'unlocked' ? badge.unlocked : !badge.unlocked);
    const categoryMatch =
              ...
              <div className='flex items-center gap-4'>
                ...
                <div className='flex-1'>
                  <h3 className='font-bold text-lg'>
                    {currentChild.name}的成就
                  </h3>
                  <p className='text-white/80 text-sm'>
                    已获得 {unlockedCount} 个勋章
                  </p>
                </div>
          ...
          <AnimatePresence mode='popLayout'>
            {filteredBadges.map((badge, index) => {
              ...
              return (
                ...
                >
                  <Card
                    className={`h-full ${levelColor.bg} ${levelColor.border} ${levelColor.shadow} shadow-lg`}
                  >
                    <CardContent className='p-4'>
                      <div className='flex flex-col items-center text-center space-y-3'>
                        <div className='relative'>
                          <div
                            className={`w-20 h-20 rounded-full bg-gradient-to-br ${levelColor.bg} flex items-center justify-center text-4xl shadow-inner ${badge.unlocked ? '' : 'opacity-50'}`}
                          >
                            {badge.icon}
                          </div>
                          {!badge.unlocked && (
                        ...
                        <div className='flex-1 w-full'>
                          <h4
                            className={`font-bold ${levelColor.text} text-sm mb-1`}
                          >
                            {badge.title}
                          </h4>
                  ...
                  <div>
                    ...
                    <div className='flex items-center gap-2 mt-1'>
                      <Badge variant='secondary'>
                        {levelIcons[selectedBadge.level]}
                      </Badge>
