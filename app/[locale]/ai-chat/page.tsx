      ...
      try {
        ...
        if (parsedMessages.length > 0) {
          const maxId = Math.max(
            ...parsedMessages.map((m: Message) => Number(m.id))
          );
          return maxId + 1;
        }
      } catch (e) {
        ...
        return [
          {
            ...
            role: 'assistant',
            content:
              '你好！我是小语的AI守护助手，我是五大AI角色的集合体。我可以帮助您记录成长、分析情绪、提供专业育儿建议。请问您想了解什么？',
            avatar: '🤖',
            name: '小语AI助手',
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      const fallbackResponse: Message = {
        ...
        role: 'assistant',
        content:
          '抱歉，我现在遇到了一些技术问题。请稍后再试，或者刷新页面重试。',
        avatar: '🤖',
        name: '小语AI助手',
        timestamp: getCurrentTimestamp(),
      };
  ...
  const generateAIResponse = (message: string, role: string) => {
    ...
    if (
      ...
      return createResponse(
        role,
        greetingResponses[role as keyof typeof greetingResponses] ??
          greetingResponses.all
      );
    }

    if (
      lowerMessage.includes('哭') ||
      lowerMessage.includes('闹') ||
      lowerMessage.includes('不安')
    ) {
      return createResponse(
        role,
        contextualResponses.listener[1] ??
          '我理解您的感受。每个孩子都有自己的成长节奏。'
      );
    }
    ...
    if (lowerMessage.includes('睡觉') || lowerMessage.includes('睡眠')) {
      return createResponse(
        role,
        contextualResponses.advisor[1] ?? '良好的睡眠习惯对宝宝成长至关重要。'
      );
    }
    ...
    if (lowerMessage.includes('发育') || lowerMessage.includes('成长')) {
      return createResponse(
        role,
        contextualResponses.recorder[1] ??
          '信息已记录。持续的记录有助于了解宝宝的成长轨迹。'
      );
    }
    ...
    if (lowerMessage.includes('安全') || lowerMessage.includes('危险')) {
      return createResponse(
        role,
        contextualResponses.guardian[0] ??
          '宝宝的安全和健康是首要关注，请确保环境安全。'
      );
    }
    ...
    if (
      ...
      lowerMessage.includes('启蒙')
    ) {
      responseIndex = 0;
      return createResponse(
        role,
        contextualResponses.cultural[responseIndex] ??
          '文化启蒙是一个循序渐进的过程，让我们一起努力。'
      );
    }

    const roleResponses =
      contextualResponses[role as keyof typeof contextualResponses] ??
      contextualResponses.all;
    responseIndex = getRandomIndex(roleResponses.length);
    const randomResponse =
      roleResponses[responseIndex] ?? '这个问题很有趣，让我来帮你分析一下。';
  ...
  const createResponse = (role: string, content: string): Message => {
    const newMessage: Message = {
      ...
      avatar:
        role === 'all'
          ? '🤖'
          : (aiRoles.find(r => r.id === role)?.avatar ?? '🤖'),
      name:
        role === 'all'
          ? '小语AI助手'
          : (aiRoles.find(r => r.id === role)?.name ?? '小语AI助手'),
      timestamp: getCurrentTimestamp(),
    };
    setMessageIdCounter(prev => prev + 1);
    return newMessage;
                ...
                <div className='flex items-center justify-between'>
                  <div className='flex items-center space-x-3'>
                    <span className='text-2xl'>
                      {selectedRole === 'all'
                        ? '🤖'
                        : aiRoles.find(r => r.id === selectedRole)?.avatar}
                    </span>
                    <div>
                      ...
                      <p className='text-sm text-gray-600'>
                        {selectedRole === 'all'
                          ? '综合五大AI角色'
                          : aiRoles.find(r => r.id === selectedRole)
                              ?.description}
                      </p>
                    </div>
                  </div>
