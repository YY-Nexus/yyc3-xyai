...
export async function POST(request: Request) {
  try {
    const { text, includeAdvice = false }: EmotionAnalysisRequest =
      await request.json();
    ...
    const detectedEmotion =
      maxScore > 0
        ? Object.keys(scores).find(key => scores[key] === maxScore) || 'neutral'
        : 'neutral';

    const emotionData =
      emotionKeywords[detectedEmotion as keyof typeof emotionKeywords];
    ...
    if (includeAdvice) {
      const adviceMap: Record<string, string> = {
        ...
        angry:
          '感到生气时，先让自己冷静下来。可以数到10，或者去喝杯水。平静后再处理问题会更好。',
        excited:
          '你的热情真棒！记得合理安排时间，保持这份激情的同时也要注意休息。',
        calm: '平和的心态很好，继续保持。这是学习和思考的最佳状态。',
        anxious:
          '感到紧张是正常的。试着做几次深呼吸，把大任务分成小步骤，一步步来就不会那么害怕了。',
        neutral:
          '今天的状态还不错。可以尝试做一些喜欢的事情，让心情更愉悦一些。',
      };
