...
const mockAssessmentResult = {
  ...
  dimensionScores: {
    academic: {
      ...
      level: '良好',
      percentile: 75,
      description: '学业基础发展良好',
    },
    learning_habits: {
      score: 82,
      level: '良好',
      percentile: 70,
      description: '学习习惯基本养成',
    },
    logical_thinking: {
      score: 90,
      level: '优秀',
      percentile: 85,
      description: '逻辑思维能力突出',
    },
    self_management: {
      score: 78,
      level: '良好',
      percentile: 65,
      description: '自我管理能力有待提升',
    },
    social_skills: {
      score: 92,
      level: '优秀',
          ...
          <div className='bg-white/70 rounded-2xl p-4 flex items-center gap-4'>
            ...
            <div className='flex-1'>
              <h3 className='font-bold text-slate-800'>
                {currentChild.name}的成长记录
              </h3>
              <p className='text-sm text-slate-500'>
                {stage?.name} · {growthStageData.exactAge?.years || 0}岁
                {growthStageData.exactAge?.months || 0}个月
              </p>
            </div>
            <ChildSelector />
          </div>
      ...
      {stageTransition && stageTransition.daysUntilTransition <= 90 && (
        <div className='bg-gradient-to-r from-amber-50 to-orange-50 rounded-2xl p-5 border border-amber-200'>
          ...
          <p className='text-sm text-amber-700'>
            ...
            天将进入
            <span className='font-bold'>
              「{stageTransition.nextStage?.name}」
            </span>
            阶段
          </p>
    ...
    >
      ...
      <div className='bg-gradient-to-r from-purple-50 to-blue-50 rounded-2xl p-5 border border-purple-100'>
        ...
        <p className='text-sm text-slate-600 leading-relaxed'>
          根据最近的评估数据，{childName}
          在语言表达和社会情感方面表现出色。建议在运动发展方面增加更多户外活动，
