...
export class AgenticCore extends EventEmitter {
  ...
  private generateSuggestedActions(type: AnalyzedIntent['type'], entities: Record<string, unknown>): string[] {
    ...
    switch (type) {
      case 'create_prediction':
        ...
        if (typeof entities.days === 'number') actions.push(`设置${entities.days}天的预测周期`)
        break
   ...
   * 执行子任务
   */
  private async executeSubtask(subtask: Subtask, task: AgentTask): Promise<any> {
    switch (subtask.type) {
   ...
   * 执行数据预处理
   */
  private async executeDataPreprocessing(_subtask: Subtask, _task: AgentTask): Promise<any> {
    // 这里实现数据预处理逻辑
    return {
      processed: true,
   ...
   * 执行评估
   */
  private async executeEvaluation(_subtask: Subtask, _task: AgentTask): Promise<any> {
    return {
   ...
   * 执行优化
   */
  private async executeOptimization(_subtask: Subtask, _task: AgentTask): Promise<any> {
    return {
      optimized: true,
   ...
   * 计算子任务置信度
   */
  private calculateSubtaskConfidence(result: any): number {
    if (!result) return 0

    // 基于结果质量计算置信度
    let confidence = 0.5

    if (result.accuracy) confidence += result.accuracy * 0.3
    if (result.processed) confidence += 0.2
    if (result.optimized) confidence += 0.2
...
class ContextManager {
  ...
  async updateContext(updates: Record<string, any>, input?: UserInput): Promise<void> {
