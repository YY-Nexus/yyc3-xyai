...
const handlers = {
  ...
  async '/api/prediction/execute'(request: Request): Promise<Response> {
    try {
      ...
      const result = await predictionService.executePrediction(
        taskId,
        data,
        horizon
      );
  ...
  async '/api/prediction/insights'(request: Request): Promise<Response> {
    try {
      ...
      const insights = await predictionService.generatePredictionInsights(
        taskId,
        results
      );
  ...
  try {
    ...
    switch (data.type) {
      case 'stream_prediction':
        const streamResult = await predictionService.executeRealTimePrediction(
          data.data
        );
        ws.send(
          JSON.stringify({
...
// 导出前端HTML
const frontendHtml = await Bun.file(
  './components/prediction/dashboard.html'
).text();
...
console.log(
  `📊 预测仪表板: http://${server.hostname}:${server.port}/dashboard`
);
