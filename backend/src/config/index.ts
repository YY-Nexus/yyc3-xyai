...
const AIConfigSchema = z.object({
  provider: z.enum(['openai', 'ollama', 'anthropic', 'custom']).default('openai'),
  openai: z.object({
    apiKey: z.string(),
...
const NotificationConfigSchema = z.object({
  channels: z.array(z.enum(['email', 'push', 'sms', 'inApp'])).default(['email', 'inApp']),
...
const MonitoringConfigSchema = z.object({
  ...
  provider: z.enum(['prometheus', 'datadog', 'newrelic', 'custom']).default('custom'),
  prometheus: z.object({
  ...
  const config = ConfigSchema.parse({
    ...
    database: {
      postgres: {
        ...
        max: process.env['DB_MAX'] ? parseInt(process.env['DB_MAX']) : undefined,
    ...
    security: {
      ...
      bcrypt: {
        rounds: process.env['BCRYPT_ROUNDS'] ? parseInt(process.env['BCRYPT_ROUNDS']) : undefined,
      },
      session: {
        ...
        cookie: {
          ...
          maxAge: process.env['SESSION_COOKIE_MAX_AGE'] ? parseInt(process.env['SESSION_COOKIE_MAX_AGE']) : undefined,
          sameSite: (process.env['SESSION_COOKIE_SAME_SITE'] as 'strict' | 'lax' | 'none') || 'lax',
        },
      },
      password: {
        minLength: process.env['PASSWORD_MIN_LENGTH'] ? parseInt(process.env['PASSWORD_MIN_LENGTH']) : undefined,
    ...
    storage: {
      ...
      local: process.env['STORAGE_PROVIDER'] === 'local' ? {
        ...
        maxFileSize: process.env['STORAGE_MAX_FILE_SIZE'] ? parseInt(process.env['STORAGE_MAX_FILE_SIZE']) : undefined,
    ...
    ai: {
      provider: (process.env['AI_PROVIDER'] as 'openai' | 'ollama' | 'anthropic' | 'custom') || 'openai',
      ...
      ollama: process.env['AI_PROVIDER'] === 'ollama' ? {
        baseURL: process.env['OLLAMA_BASE_URL'],
        model: process.env['OLLAMA_MODEL'],
        temperature: process.env['OLLAMA_TEMPERATURE'] ? parseFloat(process.env['OLLAMA_TEMPERATURE']) : undefined,
        numPredict: process.env['OLLAMA_NUM_PREDICT'] ? parseInt(process.env['OLLAMA_NUM_PREDICT']) : undefined,
      } : undefined,
      anthropic: process.env['AI_PROVIDER'] === 'anthropic' ? {
      ...
      custom: process.env['AI_PROVIDER'] === 'custom' ? {
        baseURL: process.env['CUSTOM_AI_BASE_URL'],
        apiKey: process.env['CUSTOM_AI_API_KEY'],
        model: process.env['CUSTOM_AI_MODEL'],
        headers: process.env['CUSTOM_AI_HEADERS'] ? JSON.parse(process.env['CUSTOM_AI_HEADERS']) : undefined,
      } : undefined,
      rateLimit: {
        requestsPerMinute: process.env['AI_RATE_LIMIT_PER_MINUTE'] ? parseInt(process.env['AI_RATE_LIMIT_PER_MINUTE']) : undefined,
        requestsPerHour: process.env['AI_RATE_LIMIT_PER_HOUR'] ? parseInt(process.env['AI_RATE_LIMIT_PER_HOUR']) : undefined,
        requestsPerDay: process.env['AI_RATE_LIMIT_PER_DAY'] ? parseInt(process.env['AI_RATE_LIMIT_PER_DAY']) : undefined,
      },
      cache: {
    ...
    notification: {
      channels: process.env['NOTIFICATION_CHANNELS']?.split(',') as Array<'email' | 'push' | 'sms' | 'inApp'> || ['email', 'inApp'],
      email: {
        provider: (process.env['EMAIL_PROVIDER'] as 'smtp' | 'sendgrid' | 'ses' | 'mailgun') || 'smtp',
      ...
      sms: process.env['NOTIFICATION_CHANNELS']?.includes('sms') ? {
        provider: process.env['SMS_PROVIDER'] as 'twilio' | 'nexmo' | 'aws-sns',
    ...
    logger: {
      ...
      file: {
        ...
        maxSize: process.env['LOG_FILE_MAX_SIZE'] ? parseInt(process.env['LOG_FILE_MAX_SIZE']) : 5242880,
    ...
    monitoring: {
      ...
      provider: (process.env['MONITORING_PROVIDER'] as 'prometheus' | 'datadog' | 'newrelic' | 'custom') || 'custom',
      prometheus: process.env['MONITORING_PROVIDER'] === 'prometheus' ? {
        port: process.env['PROMETHEUS_PORT'] ? parseInt(process.env['PROMETHEUS_PORT']) : undefined,
        path: process.env['PROMETHEUS_PATH'],
      } : undefined,
      datadog: process.env['MONITORING_PROVIDER'] === 'datadog' ? {
        apiKey: process.env['DATADOG_API_KEY'],
        site: process.env['DATADOG_SITE'],
      } : undefined,
      newrelic: process.env['MONITORING_PROVIDER'] === 'newrelic' ? {
      ...
      alerts: {
        ...
        errorThreshold: process.env['ALERTS_ERROR_THRESHOLD'] ? parseFloat(process.env['ALERTS_ERROR_THRESHOLD']) : undefined,
        responseTimeThreshold: process.env['ALERTS_RESPONSE_TIME_THRESHOLD'] ? parseInt(process.env['ALERTS_RESPONSE_TIME_THRESHOLD']) : undefined,
      },
    },
  });
