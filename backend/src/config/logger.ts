...
import type { LoggerConfig, LogLevel as ConfigLogLevel } from './types';

const config = getConfig();
const loggerConfig: LoggerConfig = config.logger;
...
const mapLogLevel = (level: ConfigLogLevel): string => {
  const levelMap: Record<ConfigLogLevel, string> = {
    error: 'error',
    warn: 'warn',
    info: 'info',
    http: 'http',
...
// 创建传输器
const logLevel = mapLogLevel(loggerConfig.level);
