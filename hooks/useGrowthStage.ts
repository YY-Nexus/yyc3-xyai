...
export function useGrowthStage(
  ...
  const updateStageData = useCallback(() => {
    ...
    setExactAge(
      (prev: { years: number; months: number; days: number } | null) => {
        const newAge = manager.getExactAge();
        if (!newAge) return null;
        if (
          !prev ||
          prev.years !== newAge?.years ||
          prev.months !== newAge?.months ||
          prev.days !== newAge?.days
        ) {
          return newAge;
        }
        return prev;
    ...
    setMilestoneProgress(
      (prev: { completed: number; total: number; upcoming: string[] }) => {
    ...
    setApproachingNextStage(
      (prev: {
        ...
        if (
          prev.approaching !== newApproaching.approaching ||
          prev.daysUntil !== newApproaching.daysUntil
        ) {
  ...
  const formatAge = useCallback((): string => {
    ...
    if (exactAge.years === 0 && exactAge.days > 0)
      parts.push(`${exactAge.days}天`);
