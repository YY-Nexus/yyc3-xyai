...
async function analyzeInteraction(
  record: Partial<InteractionRecord>
  ...
  return {
    keywords,
    sentiment:
      record.mood === 'excellent' || record.mood === 'good' ? '积极' : '一般',
    themes,
  ...
  return [
    {
      ...
      title: '一起读《小王子》',
      content:
        '今天和宝贝一起读了《小王子》第一章，宝贝对小王子的星球很感兴趣，问了很多问题。',
      mediaUrls: [],
      duration: 30,
      participants: ['妈妈', '宝贝'],
      location: '家里客厅',
    ...
    },
    {
      ...
      title: '手工制作母亲节贺卡',
      content:
        '宝贝用彩纸和水彩笔制作了一张母亲节贺卡，上面画了妈妈和自己手牵手。',
      mediaUrls: [],
      duration: 45,
      participants: ['妈妈', '宝贝'],
      location: '家里书房',
    ...
    async (id: string, data: Partial<InteractionRecord>) => {
      ...
      setInteractions(prev => {
        const updated = prev.map(r =>
          r.id === id ? { ...r, ...data, aiAnalysis, updatedAt: new Date() } : r
        );
        saveData(updated);
        return updated;
      });
      ...
      interactions.length > 0
        ? Math.round(
            interactions
              ...
              .reduce((sum, r) => sum + (r.aiAnalysis?.qualityScore || 0), 0) /
