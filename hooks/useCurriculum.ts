...
export function useCurriculum() {
  ...
  const getCoursesByDay = useCallback(
    (dayOfWeek: number) => {
      return courses
        .filter(
          c => c.isActive && c.schedules.some(s => s.dayOfWeek === dayOfWeek)
        )
        .map(c => ({
          ...c,
  ...
  const stats = {
    ...
    schoolCourses: courses.filter(c => c.type === 'school' && c.isActive)
      .length,
    extraCourses: courses.filter(
      c => c.type === 'extracurricular' && c.isActive
    ).length,
    onlineCourses: courses.filter(c => c.type === 'online' && c.isActive)
      .length,
    weeklyClasses: courses.reduce(
      (sum, c) => sum + (c.isActive ? c.schedules.length : 0),
      0
    ),
  };
