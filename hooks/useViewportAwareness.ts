...
export function useViewportAwareness(
  config: UseViewportAwarenessConfig = {}
): UseViewportAwarenessReturn {
  const {
    padding = 20,
  ...
  useEffect(() => {
    ...
    const updateViewport = () => {
      ...
      setViewport(prev => {
        const hasChanged =
          prev.width !== currentViewport.width ||
          prev.height !== currentViewport.height;
      ...
      try {
        ...
        console.warn(
          'useViewportAwareness: onViewportChange callback failed',
          error
        );
      }
    };
