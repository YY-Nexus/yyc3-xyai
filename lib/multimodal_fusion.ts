...
export class MultimodalEmotionFusion {
  ...
  private performFusion(
    ...
    // 加权融合valence和arousal
    const textMapping =
      EMOTION_MAPPING[textEmotion.type] || EMOTION_MAPPING.neutral;
    const voiceMapping =
      EMOTION_MAPPING[voiceEmotion.type] || EMOTION_MAPPING.neutral;
    ...
    // 归一化
    const totalWeight =
      FUSION_WEIGHTS.text * textEmotion.confidence +
      FUSION_WEIGHTS.voice * voiceEmotion.confidence;
  ...
  private calculateFusedConfidence(
    ...
    const baseConfidence =
      (textEmotion.confidence + voiceEmotion.confidence) / 2;
