...
export class GrowthStageManager {
  ...
  getAgeInMonths(): number | null {
    ...
    const now = new Date();
    return Math.floor(
      (now.getTime() - this.currentChildBirthDate.getTime()) /
        (1000 * 60 * 60 * 24 * 30.44)
    );
  }

  // 检查是否即将进入下一阶段(30天内)
  isApproachingNextStage(): {
    approaching: boolean;
    daysUntil: number;
    ...
    const daysUntil = Math.ceil(
      (nextStageDate.getTime() - now.getTime()) / (1000 * 60 * 60 * 24)
    );
  ...
  getMilestoneProgress(): {
    ...
    const completed = parsedMilestones.filter(
      m => m.monthAge <= ageInMonths
    ).length;
