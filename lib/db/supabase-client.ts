...
class MockSupabaseClient implements DatabaseClient {
  ...
  private realtimeListeners: Map<string, RealtimeCallback<unknown>[]> =
    new Map();
  ...
  >(
    ...
    return this.create<T>(table, { ...data, id } as Omit<
      T,
      'id' | 'created_at'
    >);
  }
  ...
  count(table: string, filter?: (item: unknown) => boolean): Promise<number> {
    ...
    return Promise.resolve(
      filter ? collection.filter(filter).length : collection.length
    );
  }
