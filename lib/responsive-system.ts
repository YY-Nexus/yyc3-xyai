...
export class ResponsiveSystem {
  ...
  private mediaQueries: Record<keyof typeof BREAKPOINTS, MediaQueryList> =
    {} as any;
  ...
  resolveResponsiveValue<T>(value: ResponsiveValue<T>): T | undefined {
    ...
    // 按断点优先级从高到低查找值
    const priorities: Array<keyof typeof BREAKPOINTS> = [
      '2xl',
      'xl',
      'lg',
      'md',
    ...
    for (const breakpoint of priorities) {
      if (responsiveValue[breakpoint] !== undefined) {
        ...
        if (
          breakpoint === 'xs' ||
          breakpointState[
            `is${breakpoint.charAt(0).toUpperCase()}${breakpoint.slice(1)}` as keyof BreakpointState
          ]
        ) {
