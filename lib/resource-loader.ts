...
export class ResourceLoader {
  ...
  // 收集所有资源路径
  private collectAssetPaths(
    character: CharacterConfig
  ): Array<{ path: string; type: 'default' | 'theme' | 'expression' }> {
    const paths: Array<{
      path: string;
      type: 'default' | 'theme' | 'expression';
    }> = [];
  ...
  async preloadMultipleCharacters(
    ...
    if (concurrent) {
      ...
      const loadingPromises = characters.map(async character => {
        try {
          const result = await this.loadCharacterResources(
            character,
            loadOptions
          );
...
// 导出便捷函数
export const loadCharacterResources = (
  character: CharacterConfig,
  options?: LoadingOptions
) => resourceLoader.loadCharacterResources(character, options);

export const preloadMultipleCharacters = (
  characters: CharacterConfig[],
  options?: LoadingOptions & { concurrent?: boolean }
