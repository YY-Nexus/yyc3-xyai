...
interface FullCorrectionFlowResponse {
  results: (
    | HomeworkCorrectionResponse['results'][0]
...
export class HomeworkCorrectionService {
  ...
  async smartCorrection(
    ...
    try {
      const response = await this.fetchWithRetry(
        `${this.baseURL}/agents/async-result`,
        {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
   ...
   * 完整的作业批改流程
   */
  async fullCorrectionFlow(
    imageUrl: string
  ): Promise<FullCorrectionFlowResponse> {
    try {
      // 第一步：基础批改
      const basicResult = await this.correctHomework(imageUrl);

      // 检查是否有需要智能批改的题目
      const unfinishedItems = basicResult.results.filter(
        result => !result.is_finish
      );
   ...
   * 判断是否需要对该题目进行结果解析
   */
  private shouldAnalyzeResult(
    result: HomeworkCorrectionResponse['results'][0]
  ): boolean {
    // 这里可以根据题目的特征或科目信息来判断
    // 目前简单返回true，实际应用中需要更精确的逻辑
    return (
      result.question.length > 10 || /语文|英语|文综/.test(result.question)
    );
  }
