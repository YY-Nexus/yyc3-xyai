...
export function reportWebVitals(metric: WebVitalsMetric) {
  ...
  if (process.env.NODE_ENV === 'development') {
    const color =
      rating === 'good' ? '32' : rating === 'needs-improvement' ? '33' : '31';
    logger.info(
      `\x1b[${color}m[WebVitals] ${metric.name}: ${metric.value.toFixed(2)} (${rating})\x1b[0m`
    );
  }
...
export class ResourcePreloader {
  ...
  preconnect(url: string) {
    if (
      ...
      this.loadedResources.has(`preconnect:${url}`)
    )
      return;
  ...
  dnsPrefetch(url: string) {
    if (
      typeof document === 'undefined' ||
      this.loadedResources.has(`dns:${url}`)
    )
      return;
  ...
  preload(url: string, as: 'script' | 'style' | 'image' | 'font') {
    if (
      typeof document === 'undefined' ||
      this.loadedResources.has(`preload:${url}`)
    )
      return;
  ...
  prefetch(url: string) {
    if (
      typeof document === 'undefined' ||
      this.loadedResources.has(`prefetch:${url}`)
    )
      return;
