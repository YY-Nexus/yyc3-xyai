...
import {
  getEnhancedVoiceService,
  type VoiceInteractionSession,
} from './enhanced-voice-services';
...
export class IntelligentFeedbackSystem {
  ...
  async addFeedbackData(
    data: Omit<FeedbackData, 'id' | 'timestamp'>
  ): Promise<string> {
  ...
  private calculateVoiceMetrics(
    ...
    const speakingTime = session.participants.reduce(
    ...
    patterns.forEach(pattern => {
      ...
      if (existingPattern) {
        ...
        existingPattern.effectiveness =
          (existingPattern.effectiveness + pattern.effectiveness) / 2;
  ...
  private identifyPatterns(feedback: FeedbackData): LearningPattern[] {
    ...
    // 分析情感模式
    if (
      feedback.outcomes.emotionalResponse === 'sad' ||
      feedback.outcomes.emotionalResponse === 'angry'
    ) {
      patterns.push({
   ...
   * 触发自适应策略
   */
  private async triggerAdaptiveStrategies(
    ...
    return strategy.conditions.some(condition => {
      switch (condition) {
        case 'negative_emotion_detected':
          return ['sad', 'angry', 'fearful'].includes(
            feedback.outcomes.emotionalResponse
          );
        case 'low_engagement':
  ...
  private evaluateStrategies(): void {
    for (const strategy of this.adaptiveStrategies.values()) {
      if (strategy.usageCount > 0) {
        ...
        const strategyFeedback = recentFeedback.filter(
          f =>
            f.type === 'strategy_execution' && f.data.strategyId === strategy.id
        );

        if (strategyFeedback.length > 0) {
          const successCount = strategyFeedback.filter(
            f => f.outcomes.effectiveness > 0.7
          ).length;
    ...
    this.emit(
      'strategiesEvaluated',
      Array.from(this.adaptiveStrategies.values())
    );
  }
  ...
  private updateInsights(): void {
    ...
    // 合并新洞察
    this.insights = [...this.insights.slice(-20), ...newInsights].sort(
  ...
  private analyzeEngagementTrend(): FeedbackInsight | null {
    ...
    if (trend < -0.1) {
      return {
        ...
        evidence: [
          `最近${engagementScores.length}次交互参与度下降${Math.abs(trend * 100).toFixed(1)}%`,
        ],
        recommendations: [
          '增加互动游戏性',
          '调整交互内容复杂度',
  ...
  private analyzeEmotionTrend(): FeedbackInsight | null {
    ...
    const negativeRatio =
      (emotionCounts.sad + emotionCounts.angry + emotionCounts.fearful || 0) /
      total;
    ...
    if (negativeRatio > 0.4) {
      return {
        ...
        evidence: [
          `最近${total}次交互中负面情感出现${emotionCounts.sad + emotionCounts.angry + emotionCounts.fearful || 0}次`,
        ],
        recommendations: [
          '增加安抚性互动',
          '调整语音语调更柔和',
  ...
  private analyzeLearningProgress(): FeedbackInsight | null {
    const learningFeedback = this.feedbackHistory.filter(
      f => f.type === 'learning_progress'
    );
    ...
    if (averageProgress > 0.8) {
      return {
        ...
        evidence: [
          `${learningFeedback.length}次学习活动中${progressScores.filter(s => s > 0.8).length}次表现优秀`,
        ],
        recommendations: [
          '保持当前学习策略',
          '可以适当增加学习内容难度',
    ...
    if (listeners) {
      listeners.forEach(callback => {
        try {
          ...
          console.error(
            `Error in feedback system event listener for ${event}:`,
            error
          );
        }
      });
