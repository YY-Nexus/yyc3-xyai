...
export class EnhancedVoiceService {
  ...
  private detectSpeaker(transcript: string): 'parent' | 'child' | 'ai' {
    ...
    if (childKeywords.some(keyword => lowerTranscript.includes(keyword))) {
      ...
      parentKeywords.some(keyword => lowerTranscript.includes(keyword))
    ) {
      return 'parent';
    } else {
   ...
   * 提供实时反馈
   */
  private provideRealTimeFeedback(
    transcript: string,
    speaker: string,
    emotion: string
  ): void {
  ...
  private async generateSessionInsights(): Promise<void> {
    ...
    const speakingRatio =
      session.participants.reduce((sum, p) => sum + p.speakingTime, 0) /
      (totalDuration / 1000);
  ...
  private generateSuggestions(session: VoiceInteractionSession): string[] {
    ...
    const childEmotions =
      session.participants.find(p => p.role === 'child')?.emotions || [];
    const happyRatio =
      childEmotions.filter(e => e === 'happy' || e === 'excited').length /
    ...
    const parentSpeakingTime =
      session.participants.find(p => p.role === 'parent')?.speakingTime || 0;
    const childSpeakingTime =
      session.participants.find(p => p.role === 'child')?.speakingTime || 0;
  ...
  private identifyConcerns(session: VoiceInteractionSession): string[] {
    ...
    const childNegativeCount = session.transcripts.filter(
      t => t.speaker === 'child' && negativeEmotions.includes(t.emotion)
    ).length;
    ...
    const veryShortInteractions = session.transcripts.filter(
      t => t.duration < 1
    ).length;
  ...
  ): VoiceInteractionSession['feedback'] {
  ...
  getVoiceMetrics(): VoiceMetrics {
    ...
    session.transcripts.forEach(t => {
      emotionDistribution[t.emotion] =
        (emotionDistribution[t.emotion] || 0) + 1;
    });
...
export function getEnhancedVoiceService(
