...
export class EnhancedEmotionFusion {
  ...
  private readonly INFANT_EMOTION_PATTERNS = new Map([
    ...
    [
      'hunger_cry',
      { primary: 'sadness', intensity: 0.8, valence: -0.6, arousal: 0.7 },
    ],
    [
      'pain_cry',
      { primary: 'sadness', intensity: 0.9, valence: -0.8, arousal: 0.9 },
    ],
    [
      'attention_cry',
      { primary: 'anger', intensity: 0.6, valence: -0.3, arousal: 0.8 },
    ],
    ['cooing', { primary: 'joy', intensity: 0.7, valence: 0.8, arousal: 0.4 }],
    [
      'laughing',
      { primary: 'joy', intensity: 0.9, valence: 0.9, arousal: 0.8 },
    ],
  ...
  async fuseEmotions(input: MultimodalInput): Promise<EmotionFeatures> {
    ...
    try {
      // 1. 各模态独立分析
      const textEmotion = input.text
        ? await this.analyzeTextEmotion(input.text)
        : null;
      const voiceEmotion = input.audioData
        ? await this.analyzeVoiceEmotion(input.audioData)
        : null;
      const visualEmotion = input.videoData
        ? await this.analyzeVisualEmotion(input.videoData)
        : null;
      const behavioralEmotion = input.behavioralData
        ? await this.analyzeBehavioralEmotion(input.behavioralData)
        : null;
      ...
      // 4. 年龄特化调整
      const ageAdjustedEmotion = this.ageSpecificAdjustment(
        smoothedEmotion,
        input.context?.age
      );
  ...
  private async analyzeTextEmotion(
    ...
    const emotionKeywords = new Map([
      ...
      [
        '哭',
        { primary: 'sadness', intensity: 0.8, valence: -0.7, arousal: 0.6 },
      ],
      [
        '难过',
        { primary: 'sadness', intensity: 0.7, valence: -0.6, arousal: 0.4 },
      ],
      [
        '生气',
        { primary: 'anger', intensity: 0.8, valence: -0.6, arousal: 0.7 },
      ],
      [
        '害怕',
        { primary: 'fear', intensity: 0.7, valence: -0.7, arousal: 0.8 },
      ],
      [
        '惊讶',
        { primary: 'surprise', intensity: 0.6, valence: 0.0, arousal: 0.8 },
      ],
  ...
  // 语音情感分析（增强版）
  private async analyzeVoiceEmotion(audioData: {
    features: AudioFeatures;
  }): Promise<Partial<EmotionFeatures>> {
    ...
    const emotionMapping = {
      // 高音调 + 高能量 = 兴奋/激动
      highPitchHighEnergy:
        features.pitch > 300 && features.energy > 0.7
          ? {
      ...
      // 低音调 + 低能量 = 悲伤/困倦
      lowPitchLowEnergy:
        features.pitch < 200 && features.energy < 0.3
          ? {
      ...
      // 高变化率 = 生气/沮丧
      highVariability:
        features.volumeVariability > 0.8
          ? {
  ...
  // 面部表情分析
  private analyzeFacialExpression(
    features: FacialFeatures
  ): Partial<EmotionFeatures> {
    const {
      actionUnits,
      smileIntensity,
      browRaise,
    ...
    // 基于FACS动作单元的情感识别
    if (actionUnits['AU06'] + actionUnits['AU12'] > 0.5) {
      // 嘴角上扬
      return {
        primary: 'joy',
    ...
    if (actionUnits['AU01'] + actionUnits['AU02'] > 0.5) {
      // 眉毛上扬
      return {
        primary: 'surprise',
        intensity: browRaise,
    ...
    if (eyeOpenness < 0.3 && mouthOpenness > 0.7) {
      // 哭泣表情
      return {
        primary: 'sadness',
        intensity: 0.8,
  ...
  private async analyzeBehavioralEmotion(behavioralData: {
    ...
    const activityEmotion = activityEmotionMap.get(activity) || {
      valence: 0,
      arousal: 0,
    };
  ...
  private performWeightedFusion(
    ...
    const weights = [
      this.EMOTION_WEIGHTS.text,
      this.EMOTION_WEIGHTS.voice,
      this.EMOTION_WEIGHTS.visual,
      this.EMOTION_WEIGHTS.behavioral,
    ];
  ...
  private fuseVisualEmotions(
    facial: Partial<EmotionFeatures>,
    body: Partial<EmotionFeatures>
  ): Partial<EmotionFeatures> | null {
    ...
    return {
      ...
      intensity:
        facial.intensity! * facialWeight + body.intensity! * bodyWeight,
  ...
  getEmotionTrends(windowSize: number = 10): {
    ...
    const avgValence =
      recentEmotions.reduce((sum, e) => sum + e.valence, 0) /
      recentEmotions.length;
    const avgArousal =
      recentEmotions.reduce((sum, e) => sum + e.arousal, 0) /
      recentEmotions.length;
    ...
    const primaryEmotion = [...emotionCounts.entries()].sort(
      (a, b) => b[1] - a[1]
    )[0][0];

    // 计算情感稳定性
    const valenceVariance =
      recentEmotions.reduce((sum, e) => {
