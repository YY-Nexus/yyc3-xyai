...
const getUIConfigFromGlobal = (): GlobalUIConfig => {
  try {
    ...
    // 如果获取配置失败，返回默认配置
    logger.warn(
      'Failed to get UI config from global config, using default:',
      error
    );
...
export class GlobalUIManager {
   ...
   */
  getButtonClassName(
    variant: keyof GlobalUIConfig['buttons']['variants'] = 'default',
    size: keyof GlobalUIConfig['buttons']['sizes'] = 'md',
  ...
  getSwitchClassName(disabled: boolean = false): string {
    const { disabledSwitchClassName, functionSwitchesDisabled } =
...
export const getCardClassName = () => globalUIManager.getCardClassName();

export const getAnimationClassName = (
  type: keyof GlobalUIConfig['animations']
